...
sidebar_label: Remote - Text-Embeddings-Inference
title: remote::text-embeddings-inference
---

# remote::text-embeddings-inference

## Description

HuggingFace Text-Embeddings-Inference provider for embedding models served by a TEI server via its OpenAI-compatible /v1 endpoint.
...
| `base_url` | `HttpUrl \| None` | No | http://localhost:8080/v1 | Base URL for the Text-Embeddings-Inference server (OpenAI-compatible /v1 endpoint) |

## Sample Configuration

```yaml
base_url: ${env.TEI_URL:=http://localhost:8080/v1}
```
